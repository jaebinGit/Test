...
X = sub; % 독립변수
Y1 = crossThroughPassTry; % 종속변수 1
Y2 = crossShortPassTry; % 종속변수 2
Y3 = crossDirrble; % 종속변수 3
Y4 = crossPassTry; % 종속변수 4
Y5 = crossHeadingShoot; % 종속변수 5
...
% 원하는 독립 변수 값 입력
desiredX = 0.1; % 원하는 값 입력

subplot(5, 1, 1);
scatter(X, Y1, 'o');
hold on;
plot(newX, predictedY1, '-r', 'LineWidth', 2);
title('ThroughPassTry 예측');
...
scatter(X, Y4, 'o');
hold on;
plot(newX, predictedY4, '-m', 'LineWidth', 2);
title('PassTry 예측');
...
scatter(X, Y5, 'o');
hold on;
plot(newX, predictedY5, '-c', 'LineWidth', 2);
